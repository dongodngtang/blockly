...
local mobdebug = {
  ...
  checkcount = 0, -- 检查间隔。设置成0，每次debug_hook都会去检查TCP指令，防止与运动指令等待buffer的机制耦合，导致指令执行不及时。
  yieldtimeout = 0.02, -- yield timeout (s)
  connecttimeout = 5, -- connect timeout (s)
}
...
local function update_file_line(running)
  if (socket.gettime() - currentTime > 0.1) then
    currentTime = socket.gettime()
    if currentFile then
      --print(">>>>>"..currentFile..","..currentLine..","..tostring(running).."<<<<<");
    end
  end
end
...
local function debugger_loop(sev, svars, sfile, sline)
  ...
  while true do
    ...
    elseif command == "RUN" then
      ...
      --  print("RUN:" .. "202 Paused " .. file .. " " .. tostring(line) .. "\n")
      ...
      if stream and mode and stream == "stdout" then
        ...
        genv.print = default and iobase.print or corowrap(function()
          ...
          while true do
            ...
            --serverP:send("204 Output " .. stream .. " " .. tostring(#file) .. "\n" .. file)
			--print("$$$$$$$$ file:", file)
			--serverP:send(file)
...
local function output(stream, data)
  --if server then return serverP:send("204 Output "..stream.." "..tostring(#data).."\n"..data) end
  --if server then return serverP:send(data) end
end
...
local function start(controller_host, controller_port)
  ...
  --serverP, errP = mobdebug.connect(controller_host, controller_port + 1)
  --if server and serverP then
  if server then
...
local function listen(host, port)
  ...
  if server == nil then print("Can't bind "..host..":"..tostring(port)); return end
