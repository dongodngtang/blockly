...
const defaultBlockPackages = {
  ...
  scratch3_magician: require('../blocks/scratch3_magician'),
  scratch3_magicianlite: require('../blocks/scratch3_magicianlite'),
  scratch3_controller: require('../blocks/scratch3_controller'),
  scratch3_aistarter: require('../blocks/scratch3_aistarter')
};

const UploadDevice = require('../blocks/for_upload_device');
...
/**
 * 在扩展中的积木盒里显示字符串文本
 * @param {text} label flyout 中显示的文本
 * @return {ConvertedBlockInfo} 转换后的积木信息
 */
const ConvertedLabel = label => ({
  info: {},
  json: null,
  xml: `<label text="${label}" category-label="false"></label>`
});
...
class Runtime extends EventEmitter {
  constructor() {
    ...
    this.peripheralExtensions = {
      closeDobotLink: () => DobotSerial.write(
        'dobotlink.api.CloseDobotLink'
      )
    };

    // 注册上传模式到扩展中
    this.registerPeripheralExtension(UploadDevice.extensionId, new UploadDevice.obj(this));
  ...
  static get SET_IS_CONTROLLER_MAGICIANLITE() {
    return 'SET_IS_CONTROLLER_MAGICIANLITE';
  }
  ...
  _registerBlockPackages() {
    for (const packageName in defaultBlockPackages) {
      if (defaultBlockPackages.hasOwnProperty(packageName)) {
        ...
        const packageObject = new (defaultBlockPackages[packageName])(this, packageName);
  ...
  _fillExtensionCategory(categoryInfo, extensionInfo) {
    ...
    for (const blockInfo of extensionInfo.blocks) {
      ...
      if (blockInfo.isLabel) {
        categoryInfo.blocks.push(ConvertedLabel(blockInfo.text));
        continue;
      }
      try {
  ...
  /**
     * @param {!string} deviceName whether target is Device
     * @returns {string} scratch-blocks XML description for all dynamic blocks, wrapped in <category> elements.
     */
  getBlocksXML(deviceName) {
    // if (isDevice) return [];
    const xmlParts = [];

    for (const categoryInfo of this._blockInfo) {
      if (deviceName) {
        const deviceList = categoryInfo.id.split('FOR').slice(1);
        for (let i = 0; i < deviceList.length; i++) {
          deviceList[i] = deviceList[i].toLowerCase();
        }
        if (!categoryInfo.id.includes('FOR') || !deviceList.includes(deviceName)) {
          continue;
        }
      } else if (categoryInfo.id.includes('FOR')) {
        continue;
      }
      const {
        name,
  ...
  /**
     ...
     * @param {string} extensionId - the id of the extension.
     * @returns {Promise} scan pendding result, null if no validated device instance.
     */
  scanForPeripheral(extensionId) {
    if (this.peripheralExtensions[extensionId]) {
      return this.peripheralExtensions[extensionId].scan(Runtime.PERIPHERAL_REQUEST_ERROR);
    }
    return null;
  }


  getDeviceInfoForPeripheral(extensionId) {
    if (this.peripheralExtensions[extensionId]) {
      return this.peripheralExtensions[extensionId].getDeviceInfo();
    }
    return {};
  }
  ...
  connectPeripheral(extensionId, peripheralId) {
    if (this.peripheralExtensions[extensionId]) {
      return this.peripheralExtensions[extensionId].connect(peripheralId, this._editingTarget.id)
        .then(res => {
          const {portName, deviceType, deviceId} = res;
          this.portToDeviceMap_[portName] = {
            deviceId,
            deviceType
          };
        });
  ...
  /**
     * 通过deviceid查找到对应的portName
     * @param {string} deviceId - device id
     * @returns {string} COM - portName
     */
  findKey(deviceId){
    let result = null;
    for (const key in this.portToDeviceMap_) {
      // eslint-disable-next-line no-loop-func
      Object.values(this.portToDeviceMap_[key]).forEach(item => {
        if (item === deviceId){
          result = key;
        }
      });
  ...
  /**
     ...
     * @param {string} deviceType - the id of the extension.
     * @param {string} deviceId - device id
     * @param {string} port - 端口号
     */
  disconnectPeripheral(deviceType, deviceId, port) {
    if (!deviceType || !deviceId) {
      if (port) {
        deviceId = this.portToDeviceMap_[port].deviceId;
        deviceType = this.portToDeviceMap_[port].deviceType;
      } else {
        console.log('no disconnect params enough');
      }
    }
    if (this.peripheralExtensions[deviceType]) {
      this.peripheralExtensions[deviceType].disconnect(deviceId, port);
    } else {
  ...
  getPeripheralIsConnected(extensionId) {
    ...
    if (this.peripheralExtensions[extensionId]) {
      isConnected = this.peripheralExtensions[extensionId].isConnected(this._editingTarget.id);
    }
    return isConnected;
  }
  ...
  setIsControllerMagicianlite(){
    this.isControllerMagicianLite = true;
