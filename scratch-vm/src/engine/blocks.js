...
class Blocks {
  ...
  blocklyListen (e) {
    ...
    if (e.element === 'block_disabled') {
      this.disabledBlock(e);
    }
    
    if (e.element === 'block_cancelDisable'){
      return this.cancelDisable(e);
    }
    ...
    case 'var_create':
      ...
      if (e.isLocal && editingTarget && !editingTarget.isStage && !e.isCloud) {
        ...
        window.Blockly.getMainWorkspace().getFlyout()
          .setCheckboxState(e.varId, true);
        this.emitProjectChanged();
      }
      break;
    case 'var_rename':
  ...
  disabledBlock(e) {
    ...
    this._blocks[e.blockId].disabled = true;
    this.resetCache();
  }
  ...
  cancelDisable(e){
    ...
    this._blocks[e.blockId].disabled = false;
    this.resetCache();
  }
