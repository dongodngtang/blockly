...
const fetchBitmapCanvas_ = function (costume, runtime, rotationCenter) {
  ...
  return Promise.all([costume.asset, costume.textLayerAsset].map(asset => {
    ...
    if (typeof createImageBitmap !== 'undefined') {
      if (costume.asset.isDevice) {
        return createImageBitmap(costume.asset.data, {
          type: costume.asset.assetType.contentType,
          resizeWidth: 200
        });
      }
      return createImageBitmap(
        new Blob([costume.asset.data], {type: costume.asset.assetType.contentType})
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      image.src = costume.asset.data.src || asset.encodeDataURI();
    });
  }))
...
const loadCostumeFromAsset = function (costume, runtime, optVersion) {
  ...
  costume.assetId = costume.asset && costume.asset.assetId;
