...
class Scratch3Speech2TextBlocks {
  ...
  /**
     ...
     * @private
     */
  _startListening (timeout = listenAndWaitBlockTimeoutMs) {
    console.log(timeout);
    this.runtime.emitMicListening(true);
    this._initListening().then(() => {
      // Force the block to timeout if we don't get any results back/the user didn't say anything.
      this._speechTimeoutId = setTimeout(this._stopTranscription, timeout);
    });
  }
  ...
  _newWebsocket () {
    ...
    console.log('_newWebsocket', this._newWebsocket, websocketPromise);
    return Promise.all([this._audioPromise, websocketPromise]).then(
  ...
  static get EXTENSION_ID () {
    return 'speech2textFORControllerFORMagicianLiteFORMagician';
  }
  static get EXTENSION_NAME () {
    return formatMessage({
      id: 'SpeechToText.EXTENSION_NAME',
  ...
  getInfo () {
    return {
      id: Scratch3Speech2TextBlocks.EXTENSION_ID,
      name: Scratch3Speech2TextBlocks.EXTENSION_NAME,
      ...
      blocks: [{
        ...
        text: formatMessage({
          ...
          default: 'listen and wait [time] s',
          // eslint-disable-next-line max-len
  ...
  listenAndWait (args) {
    ...
    const speechPromise = new Promise(resolve => {
      ...
      if (!listeningInProgress) {
        this._startListening(args.time * 1000);
      }
    });
