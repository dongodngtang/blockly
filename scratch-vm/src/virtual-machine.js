...
const {
  loadCostume
} = require('./import/load-costume.js');
const {
  loadSound
...
class VirtualMachine extends EventEmitter {
  constructor () {
    ...
    // 上传的文件中有MagicianLiteFORController字段时设置连接magicianBox后getpose
    this.runtime.on(Runtime.SET_IS_CONTROLLER_MAGICIANLITE, () => {
      this.emit(Runtime.SET_IS_CONTROLLER_MAGICIANLITE);
    });
    ...
    this.runtime.on(Runtime.PERIPHERAL_CONNECTED, deviceId =>
      this.emit(Runtime.PERIPHERAL_CONNECTED, deviceId)
    );
    this.runtime.on(Runtime.PERIPHERAL_REQUEST_ERROR, () =>
      this.emit(Runtime.PERIPHERAL_REQUEST_ERROR)
    );
    this.runtime.on(Runtime.PERIPHERAL_DISCONNECTED, deviceId =>
      this.emit(Runtime.PERIPHERAL_DISCONNECTED, deviceId)
    );
  ...
  scanForPeripheral (extensionId) {
    this.runtime.scanForPeripheral(extensionId, this.editingTarget.id);
  }
  ...
  loadProject (input) {
    ...
    return validationPromise
      .then(validatedInput => {
        // 上传的文件中有MagicianLiteFORController字段时设置连接magicianBox后getpose
        if (validatedInput[0].extensions &&
            validatedInput[0].extensions.find(item => item === 'MagicianLiteFORController')){
          this.runtime.emit(Runtime.SET_IS_CONTROLLER_MAGICIANLITE);
        }
        this.deserializeProject(validatedInput[0], validatedInput[1]);
      })
      .then(() => {
        this.runtime.peripheralExtensions.magician.disconnectAll();
        this.runtime.peripheralExtensions.magicianlite.disconnectAll();
        this.runtime.peripheralExtensions.controller.disconnectAll();
  ...
  deserializeProject (projectJSON, zip) {
    ...
    const deserializePromise = function () {
      ...
      if (projectJSON.toLoadAIData) {
        window.toLoadAIData = projectJSON.toLoadAIData;
      }
      if (projectJSON.variableList) {
        window.variableList = projectJSON.variableList;
      }
      if (projectVersion === 2) {
  ...
  installTargets (targets, extensions, wholeProject) {
    ...
    return Promise.all(extensionPromises).then(() => {
      ...
      this.emitTargetsUpdate(false /* Don't emit project change */, true);
      this.emitWorkspaceUpdate();
  ...
  deleteSprite (targetId) {
    ...
    if (target) {
      let targetIndexBeforeDelete; let selecteNextId;
      const deviceTargets = this.runtime.targets.filter(t => t.isDevice);
      const spriteTargets = this.runtime.targets.filter(t => !t.isDevice);
      if (target.isDevice) {
        targetIndexBeforeDelete = deviceTargets.map(t => t.id)
          .indexOf(target.id);
        if (targetIndexBeforeDelete === 0) {
          // 删除的是第一个硬件
          targetIndexBeforeDelete = 1;
        } else {
          targetIndexBeforeDelete -= 1;
        }
        if (deviceTargets.length > 1) {
          selecteNextId = deviceTargets[Math.min(targetIndexBeforeDelete, deviceTargets.length - 1)].id;
        } else if (spriteTargets.length > 0){
          selecteNextId = spriteTargets[spriteTargets.length - 1].id;
        }
      } else {
        targetIndexBeforeDelete = spriteTargets.map(t => t.id)
          .indexOf(target.id);
        // 包含了一个 stage, 所以 length 要加 1
        if (spriteTargets.length > 2) {
          selecteNextId = spriteTargets[Math.min(targetIndexBeforeDelete, spriteTargets.length - 1)].id;
        } else if (deviceTargets.length > 0) {
          selecteNextId = deviceTargets[deviceTargets.length - 1].id;
        }
      }
      if (!target.isSprite()) {
      ...
      for (let i = 0; i < sprite.clones.length; i++) {
        ...
        if (clone === currentEditingTarget) {
          if (this.runtime.targets.length > 0) {
            if (selecteNextId) {
              this.setEditingTarget(selecteNextId);
            } else {
  ...
  setLocale (locale, messages) {
    const m = Object.create(messages);
  ...
  /**
     ...
     * @param {bool} isAddSprite 添加的是角色还是设备
     * Disabled selectively by updates that don't affect project serialization.
     * Defaults to true.
     */
  emitTargetsUpdate (triggerProjectChange, isAddSprite) {
  ...
  /**
     ...
     */
  emitWorkspaceUpdate () {
    if (this.runtime.getTargetForStage() === undefined) {
      return;
    }
    ...
    let defaultXml = '';
    const arduinoDevices = ['aistarter', 'mobileplatform'];
    const deviceName = this.editingTarget.deviceName;
    if (deviceName && !this.editingTarget.blocks.toXML() && (
      arduinoDevices.includes(deviceName) ||
      deviceName.includes('arduino'))) {
      defaultXml = `
        <block type="Arduino_Start" x="45" y="82"><next>
        <block type="control_forever"></block></next></block>
        `;
    }
  ...
  /**
   * For search Dobot from dobotlink api
   * @returns {Promise} scan Promise;
   */
  searchDobot () {
    return this.runtime.scanForPeripheral(this.editingTarget.deviceName, this.editingTarget.id);
  }
  /**
   * To connect dobot by port
   * @param {string} port portName to connect
   */
  connectDobot (port) {
    return this.runtime.connectPeripheral(this.editingTarget.deviceName, port);
  }
  /**
   * To disconnect Dobot
   * @param {string} port portName to disconnect
   * @param {boolean} isEditingTarget if not editing target, will use different devicename and id;
   */
  disConnectDobot (port, isEditingTarget = true) {
    const deviceName = isEditingTarget ? this.editingTarget.deviceName : null;
    const targetId = isEditingTarget ? this.editingTarget.id : null;
    this.runtime.disconnectPeripheral(deviceName, targetId, port);
  }
  /**
   * upload generated code to device
   * @param {string} code generated C++ code
   * @param {fn} callback callback after uploaded
   */
  uploadDevice (code, callback) {
    const deviceId = this.editingTarget.id;
    this.runtime.peripheralExtensions.uploadDevice.uploadCode(code, deviceId, callback);
  }
  /**
   * choose one upload portName for upload in the future.
   * @param {string} portName portName to upload
   */
  chooseUploadPort (portName) {
    const deviceId = this.editingTarget.id;
    this.runtime.peripheralExtensions.uploadDevice.choosePort(deviceId, portName);
  }
  /**
   * To check device is binding to port or not
   * @returns {boolean} is connect or not
   */
  isDeviceBindPort () {
    const deviceId = this.editingTarget.id;
    return this.runtime.peripheralExtensions.uploadDevice.isDeviceBindPort(deviceId);
  }
}
