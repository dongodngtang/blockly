...
const serializeTarget = function (target, extensions) {
  ...
  obj.isDevice = target.isDevice;
  obj.isUpload = target.isUpload;
  obj.deviceName = target.deviceName;
  ...
  } else if (obj.isDevice) { // The stage does not need the following properties, but sprites should
    obj.visible = false;
    obj.size = 0;
  } else {
...
const serialize = function (runtime, targetId) {
  ...
  if (window.toSaveAIData) {
    obj.toLoadAIData = window.toSaveAIData;
  }
  // 添加aistarter变量数据
  if (window.variableList){
    obj.variableList = window.variableList;
  }
...
const parseScratchObject = function (object, runtime, extensions, zip) {
  ...
  if (object.hasOwnProperty('blocks')) {
    ...
    for (const blockId in object.blocks) {
      ...
      if (/^(AIVoice|AIBlock|String)/.test(blockJSON.opcode)) {
        window.AIAdded = true;
      }
      if (/^(PhotoelectricSensor|String)/.test(blockJSON.opcode)) {
        window.photoelectricAndColorSensorAdded = true;
      }
      if (/^(SlidingRail|String)/.test(blockJSON.opcode)) {
        window.slidingRailXmlAdded = true;
      }
      if (extensionID) {
  ...
  if (object.hasOwnProperty('isUpload')) {
    target.isUpload = object.isUpload;
  }
  if (object.hasOwnProperty('deviceName')) {
    target.deviceName = object.deviceName;
  }
