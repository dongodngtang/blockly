...
/**
 * 输出和单个字段的shadow 类积木的最小高度
 * 将仅仅被用作包含一个字段的 shadow 积木, 这将会比 reporter 更小
 * @const
 */
Blockly.BlockSvg.MIN_BLOCK_Y_SINGLE_FIELD_OUTPUT = 8 * Blockly.BlockSvg.GRID_UNIT;

/**
 * 输出类 non-shadow 积木的最小高度, 比如 reporter
 * @const
 */
...
/**
 * 声明输入的左下角SVG path
 * 包含里面的圆角Includes the rounded inside corner.
 * @const
 */
...
/**
 * 与之前积木连接的最小积木的输入和字段的 X值
 * 保证输入不会被上面积木的缺口遮盖Ensures that inputs will not overlap with the top notch of blocks.
 * @const
 */
...
/**
 * 输入输出形状的 Mao, 和相关的 padded 大小
 * 外部 key 对应外部的形状, 内部key 对应内部的形状
 * 当有外部形状的积木包含了内部形状的输入积木
 * 在他的左边或者右边, 那一边会被扩展开
 ...
 * 渲染积木
 * 布局并且重绘积木, 基于内容和设置
 * @param {boolean=} opt_bubble 如果是 false, 则只渲染积木
 *   如果是 true, 机会渲染积木的所有父, 祖父元素, 默认为 true
 */
Blockly.BlockSvg.prototype.render = function(opt_bubble) {
  ...
  // 如果没有 icons, cursorX 将会是0, 否则将会是第一个标签需要移动的宽度
 ...
 * 画 block 的 path
 * 移动 fields 到正确的地方
 * @param {number} iconWidth 第一行因为 icons 的偏移量
 * @param {!Array.<!Array.<!Object>>} inputRows 二维数组对象, 每个包含位置信息
 * @private
 */
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
  this.startHat_ = false;
  // 左上角是圆角还是曲线, 目前只有 hat 积木时曲线
  ...
  // 所需控件从 top 画到 bottom
  // 为to make room for the left and right to draw shapes (curves or angles).
