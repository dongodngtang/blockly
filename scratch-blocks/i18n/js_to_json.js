...
const DEVICE_PATH_INPUT = path.resolve(__dirname, '../msg/device_msg/message.js');
...
const match = function(str) {
  if (str.indexOf('Blockly.Msg.') === -1) return false;
...
const stream2 = fs.createReadStream(DEVICE_PATH_INPUT);
stream2.
    pipe(es.split('\n'))
    .pipe(es.mapSync(function(str) {
      if (!match(str)) return;
