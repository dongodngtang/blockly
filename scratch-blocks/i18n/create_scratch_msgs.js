...
let zhcn = fs.readFileSync(path.resolve(__dirname, '../msg/json/zh-cn.json'));
zhcn = JSON.parse(zhcn);
const zhcnKeys = Object.keys(zhcn).sort().toString();
...
const validate = function(json, name) {
  const compare_obj = name === 'en' ? enKeys : zhcnKeys;
  assert.strictEqual(Object.keys(json).sort().toString(), compare_obj, `${name}: Locale json keys do not match en.json`);
