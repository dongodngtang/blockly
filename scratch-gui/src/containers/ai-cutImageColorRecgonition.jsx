...
class CutImageColorRecgonition extends React.Component {
  state = {
    rectArray: null,
    isShowLoading: false
  };
    ...
    if (this.props.step === 0) {
      ...
      this.props.imageListObj.map((classImageList, classIndex) => {
        ...
        classImageList.map((image, imageIndex) => {
          ...
          this.props.caliWs.colorImageClassify([image], [classIndex], size, flag).then(res => {
            if (flag === 2) {
  ...
  onCaptureClick = (classIndex, data) => {
    ...
    this.props.handleSetCutColorImageListObj(tempImageList);
  }
  ...
  onCalibrationValidateImgChange = (image, isValidateCutImage, isCutRecognizeBlock) => new Promise(resolve => {
    ...
    // 如果是第一步点击分割时
    if (this.props.step === 0) {
      this.props.caliWs.colorImageCut(image).then(rectArray => {
        if (!this.isUnmount){
          this.setState({
            rectArray
          }, resolve);
      ...
      if (isValidateCutImage) {
        this.props.caliWs.colorImageGroup(image).then(index => {
          resolve(index);
        });
      } else if (this.props.caliWs.colorImageCut(image, !isCutRecognizeBlock)){
        this.props.caliWs.colorImageCut(image, !isCutRecognizeBlock).then(rectArray => {
  ...
  onCharacterNameChange = (classIndex, value) => {
    ...
    this.props.handleSetCutColorClassNames(tempCharacterNames);
  }
  ...
  delClass = delClassIndex => {
    ...
    this.props.handleSetCharacterNames(tempCharacterNames);
    this.props.handleSetCutColorImageListObj(tempImgList);
    this.props.handleSetCutColorClassNames(tempCharacterNames);
  }
  ...
  addClass = () => {
    ...
    const newClass = (isFacial ? 'Name' : 'Color') + (++this.classNum);
  ...
  delClassificationData = () => {
    this.props.changeIsShowDlePopup().then(() => {
      ...
      this.props.handleSetCharacterNames(['Color1']);
      this.props.handleSetCutColorImageListObj([[]]);
      this.props.handleSetCutColorClassNames(['Color1']);
      this.props.turnIsShowDlePopup(false);
      
      if (window.toSaveAIData){
        window.toSaveAIData.cutColorImageListObj = [[]];
        window.toSaveAIData.cutColorClassNames = [];
      }
    })
      .catch(() => {
        this.props.turnIsShowDlePopup(false);
  ...
  render = () => (
    <AI
      ...
      isColorCut={this.props.isColorCut}
...
CutImageColorRecgonition.propTypes = {
  x: PropTypes.number,
  y: PropTypes.number,
...
export default connect(mapStatesToProps, mapDispatchToProps)(CutImageColorRecgonition);
