...
import dobotLogo from '../menu-bar/dobot-logo.png';
...
let count = 0;
let timer;
const handleOpenDevClick = function (e) {
  if (!window.require) return;
  if (timer) {
    if (count < 5) {
      count++;
    } else {
      window.require('electron').ipcRenderer.send('open-dev-tool');
      count = 0;
    }
  } else {
    timer = setTimeout(() => {
...
const StageHeaderComponent = function (props) {
  ...
  if (isFullScreen) {
    ...
    const stageButton = showBranding ? (
      <div className={styles.embedDobotLogo}>
        <a
          href="https://dobot.cc"
    ...
    const stageControls =
            isPlayerOnly || (window.innerHeight < 800) ? (
              []
            ) : (
