...
import { removeConnectedDevice } from '../reducers/connect_status';
...
const vmListenerHOC = function (WrappedComponent) {
  class VMListener extends React.Component {
    constructor (props) {
      ...
      // 为了在连接断开时调用
      this.props.vm.on('PERIPHERAL_DISCONNECTED', this.props.onRemoveConnectedDevice);
    ...
    handleTargetsUpdate (data) {
      if (this.props.shouldEmitUpdates && data.editingTarget) {
        const target = this.props.vm.runtime.getTargetById(data.editingTarget);
        // 初始加载角色时
        let deviceId;
        let spriteId;
        if (data && !this.props.currentDeviceTab && !this.props.currentSpriteTab) {
          data.targetList.forEach(t => {
            if (t.isDevice) {
              deviceId = t.id;
            } else {
              spriteId = t.id;
        ...
        // 如果是加载项目或初始进入项目, 设置 id 为该类型最后一个角色的 id
        if (deviceId || spriteId) {
          if (deviceId) {
            this.props.setDeviceTab(deviceId);
            this.props.vm.setEditingTarget(deviceId);

          }
          if (spriteId) {
            this.props.setSpriteTab(spriteId);
          }
        } else if (data.isAddSprite) {
          // 如果是添加设备
          if (target.isDevice) {
            this.props.setDeviceTab(target.id);
          } else {
            this.props.setSpriteTab(target.id);
          }
        }
  ...
  VMListener.propTypes = {
    ...
    currentDeviceTab: PropTypes.string,
    currentSpriteTab: PropTypes.string,
  ...
  const mapStateToProps = state => ({
    ...
    currentDeviceTab: state.scratchGui.targetTab.device_tab_select,
    currentSpriteTab: state.scratchGui.targetTab.sprite_tab_select
  });
  const mapDispatchToProps = dispatch => ({
    ...
    onRemoveConnectedDevice: deviceId => {
      dispatch(removeConnectedDevice(deviceId));
    }
  });
  return connect(
    mapStateToProps,
